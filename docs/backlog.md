...
| D-66 | **E-16 — Tableau de bord du responsable dans `planning.html`** (`server.js` + `planning.html`) — onglet dynamique **« 📋 Tableau de bord »** ajouté avant l'onglet Dispos pour tout staff porteur d'un rôle de type `responsable` ayant au moins un shift sur la semaine en cours. Nouvel endpoint `GET /api/me/responsable-week?from=…&to=…` qui (1) valide le rôle responsable, (2) collecte tous les couples (date, établissement) où le staff travaille, (3) renvoie tous les shifts de l'équipe sur ces couples groupés par date. **Élargi mai 2026** : retrait du filtre `isResponsablePourSoiree()` — un responsable voit désormais l'équipe sur **toutes** ses soirées de travail, et plus seulement celles où il est désigné `pointage_resp`. Rendu : table style tableau de bord patron (staff × jours, pastilles `start-end` colorées + chip nom d'établissement sous chaque pastille), lecture seule, semaine en cours. *(Note : E-17 tentait la même idée pour les directeurs dans `index.html` — reverté `994b2ff`, mauvaise cible.)* | (à commit) |
