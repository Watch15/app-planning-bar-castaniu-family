...
| D-53 | **Vue staff — toggle Semaine/Mois + stats Historique** (`planning.html`) — segmented control « Semaine / Mois » au-dessus de `#week-stats`, défaut Semaine. Mode **Mois** : nouvelles fonctions `loadMonthRecap()` (fetch parallèle mois en cours + mois précédent via la route existante `/api/my-shifts?from=&to=`, **pas de nouvelle route backend**) et `renderMonthStats()` (3 cartes Jours/Shifts/Heures + delta « vs mois préc. »). Mois calendaire (frontière à minuit — à revisiter pour shifts 18h-2h). État `_lastWeekData`/`_lastMonthData` pour switch instantané sans re-fetch. Greeting sub-text adapté (`Récap de <mois>`). **Onglet Historique** : fetch parallèle semaine antérieure, cache enrichi `{ data, prevTotalH }`, `buildHistStatsHtml()` insère un bloc stats (3 cartes + delta « vs sem. préc. ») au-dessus de la liste des cartes journées, avec **répartition par établissement** si > 1 estab. Chips établissement : **pastilles colorées retirées** sur les 3 récaps (semaine/mois/historique) — affichage `nom + heures` uniquement, CSS `.estab-hours-dot` + variable `color` du destructuring supprimés. Règles respectées : aucun `toISOString()`, Jokers exclus, modifs additives à `planning.html`. | (à commit) |
