...
|---|---|---|
| R-01 | Unifier le calcul du lundi de semaine : `getMondayOf()` (front) ré-implémente `weekStart()` (`lib/utils.js`) → extraire en module UMD partagé + tests, supprimer la duplication | P2 |
| R-02 | Extraire la logique de publication (`isAutoPublished` + lecture des flags `publish_<week>`) hors du front pour une source unique | P3 |
...
| C-01 | Agenda (D-72) : la carte « Ajouter à mon agenda » renvoie une erreur pour un directeur **sans `staff_id`** → masquer la carte si pas de profil staff lié | P2 |
| C-02 | Agenda (D-72) : envisager `PUBLIC_BASE_URL` (env Railway) pour figer le domaine des URLs `.ics` en cas de domaine custom | P3 |
| C-03 | `/api/dispos/non-affectees` **à recréer** en excluant `type:'off'` (référencé dans les notes ci-dessous) | P2 |
...
- **Tests** : `npm test` (zéro dépendance, `node --test`) — **49 tests, 2 suites** (`tests/utils.test.js` + `tests/shift-hours.test.js`). ⚠️ le mode répertoire `node --test tests/` **n'est pas fiable** (échoue selon la version Node) : lister les fichiers explicitement dans `package.json`. Ajouter un test quand on extrait un helper pur, change une règle de date/heure, ou fixe un bug qui pourrait régresser.
- **Logique partagée navigateur/Node (D-73)** : pour qu'un helper soit à la fois consommé par le front (`<script src>`) ET testable sous Node, le mettre dans un **module UMD** sous `public/lib/` (ex. `shift-hours.js` → `window.ShiftHours` navigateur + `require()` Node). Côté HTML, déléguer depuis une fonction de même nom pour préserver le hoisting et ne pas toucher les call sites. C'est le gabarit des prochaines extractions (R-01/R-02).
