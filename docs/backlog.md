...
| D-66 | **E-16 — Tableau de bord du responsable dans `planning.html`** (`server.js` + `planning.html`) — onglet dynamique **« 📋 Tableau de bord »** ajouté avant l'onglet Dispos uniquement pour un staff désigné responsable (`pointage_resp:true` sur l'un de ses shifts de la semaine) ET porteur d'un rôle de type `responsable`. Nouvel endpoint `GET /api/me/responsable-week?from=…&to=…` qui (1) valide ces deux conditions, (2) collecte les (date, établissement) où le staff est *le* responsable de la soirée via `isResponsablePourSoiree()`, (3) renvoie tous les shifts de l'équipe sur ces couples groupés par date. Rendu : table style tableau de bord patron (staff × jours, pastilles `start-end` colorées + chip nom d'établissement sous chaque pastille), lecture seule, semaine en cours. *(Note : E-17 tentait la même idée pour les directeurs dans `index.html` — reverté `994b2ff`, mauvaise cible.)* | (à commit) |
