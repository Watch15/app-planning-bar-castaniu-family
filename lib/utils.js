...
// Nettoie + valide une liste de jours OFF déclarés par un directeur.
// Déduplique, ne garde que le format YYYY-MM-DD, refuse tout jour passé.
// Retourne { clean: string[], error: string|null }.
function cleanOffDates(dates, today) {
    const arr   = Array.isArray(dates) ? dates : (dates ? [dates] : []);
    const clean = [...new Set(arr)].filter(d => ISO_DATE_RE.test(d));
    if (clean.length === 0) return { clean: [], error: 'Aucune date valide fournie (format YYYY-MM-DD)' };
    if (clean.some(d => d < today)) return { clean: [], error: 'Une absence doit être à venir.' };
...
function scopeManagerOff(offs, metaById, viewer, canAccess) {
    ...
    for (const o of offs) {
        ...
        if (canSee) out.push({
            _id: o._id, user_id: o.user_id, date: o.date, type: o.type || 'off',
